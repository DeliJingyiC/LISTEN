...
# Default experiments if none specified (text-only model)
if [ -z "$EXPERIMENTS" ]; then
    EXPERIMENTS="1_text,2A,3A"
fi
...
python -u /users/PAS2062/delijingyic/project/LISTEN/scripts/test_qwen3_instruct.py \
  --model "$MODEL_KIND" \
