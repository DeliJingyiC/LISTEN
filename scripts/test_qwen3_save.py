...
"""
Fixed Qwen3-Omni test without data leakage
...
def get_checkpoint_filename(experiment_type, input_mode, model_prefix="qwen3_omni"):
...
def inference_with_audio_clean(audio_array, sr, prompt, sys_prompt="You are Qwen, a virtual human developed by the Qwen Team, Alibaba Group, capable of perceiving auditory and visual inputs, as well as generating text and speech."):
    """
    Clean inference function that doesn't leak information - Qwen3 version
    """
    ...
    print("Converted inputs to bfloat16")
    print("model.device: ", model.device)
    print("Finished processing inputs")
    

    torch.cuda.empty_cache()
    # Generate with controlled parameters - Qwen3 specific parameters
    ...
    # Decode output - Qwen3 returns different structure
...
def inference_with_audio_and_text_clean(audio_array, sr, transcription, prompt, sys_prompt="You are Qwen, a virtual human developed by the Qwen Team, Alibaba Group, capable of perceiving auditory and visual inputs, as well as generating text and speech."):
    """
    Clean inference function for experiments 2C and 3C with both audio and transcription - Qwen3 version
    """
    ...
    # Convert inputs to bfloat16 - CRITICAL FIX
    for key, value in inputs.items():
        if isinstance(value, torch.Tensor) and value.dtype == torch.float32:
            inputs[key] = value.to(torch.bfloat16)
    print("Converted inputs to bfloat16")
    
    # Generate with Qwen3 parameters
    text_ids, audio_output = model.generate(
        **inputs,
        thinker_return_dict_in_generate=True,
        thinker_max_new_tokens=3,
...
def inference_with_text_clean(text_input, prompt, sys_prompt="You are Qwen, a virtual human developed by the Qwen Team, Alibaba Group, capable of perceiving auditory and visual inputs, as well as generating text and speech."):
    """
    Clean inference function for text-based inputs (experiments 2A, 3A) - Qwen3 version
    """
...
def test_single_experiment(experiment_type, input_mode="audio", model_kind="qwen3", resume=True):
    ...
    for sample_idx in range(start_idx, total_samples):
        ...
        if expected_letter is None:
            ...
            if input_mode == "text" and experiment_type in ["3A"]:
...
Respond with only the letter (A, B, C, D, E, F, G, H):"""
        ...
        ground_truth = original_sample.get('explicit_emotion', original_sample['answer']) if input_mode == "text" and experiment_type in ["3A"]else original_sample['answer']
        ...
        try:
            if input_mode == "audio":
                ...
                if model_kind == "qwen3":
                    # Get prediction with text using Qwen3
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="qwen3", choices=["qwen3"], 
                       help="Model to use: qwen3 (all experiments)")
