#!/usr/bin/env python3
"""
Gemini 2.5 Flash test for VibeCheck experiments
...
def load_checkpoint(checkpoint_file):
    ...
    if os.path.exists(checkpoint_file):
        try:
            ...
            # Recompute progress from results, trimming trailing 429 RESOURCE_EXHAUSTED entries
            last_non_quota_idx = -1
            for i, r in enumerate(prev_results):
                err = r.get('error')
                if not err or ("429" not in str(err) or "RESOURCE_EXHAUSTED" not in str(err)):
                    last_non_quota_idx = i

            if last_non_quota_idx + 1 < len(prev_results):
                print(
                    f" Trimming {len(prev_results) - (last_non_quota_idx + 1)} trailing 429 failures from checkpoint results"
                )
                prev_results = prev_results[: last_non_quota_idx + 1]
            ...
            # Resume index is the count of valid results
            return (
                prev_results,
                len(prev_results) - 1,
    ...
    try:
        ...
        if m:
            ...
            if candidates:
                ...
                # Determine last non-429 error index to resume after
                last_non_quota_idx = -1
                for i, r in enumerate(prev_results):
                    err = r.get('error')
                    # Treat entries without error or with non-429 errors as valid progress points
                    if not err or ("429" not in str(err) or "RESOURCE_EXHAUSTED" not in str(err)):
                        last_non_quota_idx = i

                # Trim trailing 429 failures if present so stats and accuracy reflect only valid progress
                if last_non_quota_idx + 1 < len(prev_results):
                    trimmed_len = last_non_quota_idx + 1
                    print(
                        f" Trimming {len(prev_results) - trimmed_len} trailing 429 failures from results before resuming"
                    )
                    prev_results = prev_results[:trimmed_len]

                resume_from = last_non_quota_idx + 1 if last_non_quota_idx >= 0 else 0

                print(f" No checkpoint found; resuming from latest results file: {os.path.basename(latest)}")
                print(
                    f" Found {len(prev_results)} non-429 samples in results; will resume from index {resume_from}"
                )
                return prev_results, last_non_quota_idx, experiment_info
    except Exception as e:
...
def get_checkpoint_filename(experiment_type, input_mode, model_prefix="gemini2_5_flash"):
...
def inference_with_audio_clean(client, audio_array, sr, prompt, sys_prompt="You are a helpful AI assistant capable of analyzing audio and text to classify emotions accurately."):
    """
    Clean inference function for audio-only inputs using Gemini 2.5 Flash
    """
    try:
        # Prepare audio part
...
def inference_with_audio_and_text_clean(client, audio_array, sr, transcription, prompt, sys_prompt="You are a helpful AI assistant capable of analyzing audio and text to classify emotions accurately."):
    """
    Clean inference function for audio+text inputs using Gemini 2.5 Flash
    """
    try:
        # Prepare audio part
        audio_part = prepare_audio_part(audio_array, sr)
...
def inference_with_text_clean(client, text_input, prompt, sys_prompt="You are a helpful AI assistant capable of analyzing text to classify emotions accurately."):
    """
    Clean inference function for text-only inputs using Gemini 2.5 Flash
    """
    try:
        # Combine transcription with prompt
...
def find_resume_log(directory_path, experiment_type, input_mode):
    """Scan a directory for .log files that contain the experiment banner.

    Returns the most recently modified matching log path, or None.
    """
    try:
...
def test_single_experiment(client, experiment_type, input_mode="audio", resume=True, resume_from_log=None):
    ...
    """
    print(f"Testing Gemini 2.5 Flash on Experiment {experiment_type} ({input_mode})...")
    ...
    # Optional: override resume index using a log file BUT ONLY if there is no
    # progress from checkpoint or results (i.e., start_idx == 0). Results/checkpoints take precedence.
    if resume_from_log:
...
Respond with only the letter (A, B, C, D, E, F, G, H):"""
    ...
    output_file = f"results/gemini2_5_flash_experiment{experiment_type}_{input_mode}_results_3allemo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def test_all_experiments(resume=True, resume_from_log=None, auto_resume_from_logs=False, logs_dir=None):
    ...
    print("Testing Gemini 2.5 Flash on Experiments WITHOUT data leakage...")
    ...
    for exp_type, input_mode in experiments:
        ...
        try:
            ...
            if not chosen_log and auto_resume_from_logs and logs_dir:
                candidate = find_resume_log(logs_dir, exp_type, input_mode)
                if candidate:
                    print(f" Auto-selected resume log for {exp_type}({input_mode}): {os.path.basename(candidate)}")
                    chosen_log = candidate

            results = test_single_experiment(client, exp_type, input_mode, resume=resume, resume_from_log=chosen_log)
    ...
    combined_output_file = f"results/gemini2_5_flash_all_experiments_results_3allemo_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def run_selected_experiments(experiments_to_run=None, resume=True, resume_from_log=None, force_input_mode=None, auto_resume_from_logs=False, logs_dir=None):
    ...
    print("Testing Gemini 2.5 Flash on selected experiments WITHOUT data leakage...")
    ...
    combined_output_file = f"results/gemini2_5_flash_{exp_suffix}_experiments_results_3allemo_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(combined_output_file, 'w') as f:
        json.dump(all_results, f, indent=2, default=str)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run VibeCheck experiments with Gemini 2.5 Flash")
    ...
    if args.list_experiments:
        print(f"\nAvailable experiments for Gemini 2.5 Flash:")
        experiments_info = {
    ...
    print("Testing Gemini 2.5 Flash with proper controls for data leakage")
